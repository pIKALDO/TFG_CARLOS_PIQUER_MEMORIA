...
Además, el sistema integra un \textit{bot de Telegram} encargado de tareas como el envío de notificaciones post-compra, la generación automática de facturas o la detección de carritos abandonados.  
...
También se representa la respuesta automatizada del \textit{bot de Telegram} para confirmar la compra, descontar el stock y generar la factura.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figs/1CompraEstandar.png}
...
\textbf{Postcondiciones} &
El pago se confirma, el pedido se marca como “pagado” y se notifica al bot de Telegram. \\ \hline
...
7. La WebApp notifica al bot de Telegram para las tareas post-compra. \\ \hline

\textbf{Flujos alternativos} &
5.a Stripe requiere autenticación adicional (3D Secure). \newline
7.a Error notificando al bot de Telegram → reintento automático. \\ \hline
...
Una vez validado el pago, el sistema genera el documento asociado y lo envía al cliente mediante el \textit{bot de Telegram}, garantizando así un flujo de trabajo totalmente automatizado.
...
\textbf{Actor(es)} & WebApp, Bot de Telegram \\ \hline

\textbf{Descripción} &
Una vez confirmado el pago del pedido, la WebApp notifica al bot de Telegram, que genera la factura correspondiente y la envía al cliente. \\ \hline
...
\textbf{Flujo principal} &
1. La WebApp notifica al bot de Telegram indicando que el pedido está pagado. \newline
2. El bot de Telegram genera la factura. \newline
3. El bot de Telegram almacena o registra la factura en el sistema. \newline
4. El bot de Telegram envía la factura por mensaje al cliente. \\ \hline

\textbf{Flujos alternativos} &
2.a Error generando la factura → el bot de Telegram programa un reintento. \\ \hline

\textbf{Requisitos funcionales relacionados} & RF-06, RF-07 \\ \hline
\end{tabular}
...
\textbf{Flujo principal} &
1. El bot de Telegram ejecuta una tarea programada. \newline
2. Identifica los carritos inactivos. \newline
3. Marca estos carritos como abandonados. \newline
4. Libera stock en la base de datos. \\ \hline

\textbf{Flujos alternativos} &
4.a Error actualizando stock → el bot de Telegram registra el error. \\ \hline
...
Cuando el inventario de un producto alcanza un umbral mínimo, el sistema ejecuta una tarea automatizada que envía un aviso al administrador mediante el \textit{bot de Telegram}, permitiendo reponer el producto a tiempo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figs/Bajo_stockgood.jpg}
...
\textbf{Actor(es)} & WebApp, Bot de Telegram, Administrador \\ \hline

\textbf{Descripción} &
Cuando el nivel de stock de un producto cae por debajo del umbral configurado, el bot de Telegram envía una notificación al administrador. \\ \hline
...
2. Notifica al bot de Telegram. \newline
3. El bot de Telegram verifica si el stock está por debajo del umbral. \newline
4. El bot de Telegram envía una alerta al administrador. \\ \hline
...
El modelo relacional resultante se compone de las siguientes tablas principales:
\texttt{usuarios}, \texttt{productos}, \texttt{carritos}, \texttt{carrito\_items},
\texttt{pedidos}, \texttt{pagos} y \texttt{direcciones}. La Figura~\ref{fig:prototipo-bd}
muestra el esquema inicial diseñado durante la fase de análisis, elaborado mediante
la herramienta \textit{dbdiagram.io}.

Este prototipo sirvió como base para el desarrollo de la base de datos definitiva,
que fue refinada durante la implementación para incorporar elementos adicionales
derivados del análisis de requisitos. Entre los principales ajustes realizados
destacan la incorporación de la tabla \texttt{categorias} para la clasificación
de productos, el atributo \texttt{stock\_minimo} en la tabla \texttt{productos}
---necesario para la lógica de alertas del bot de Telegram---, el atributo
\texttt{precio\_unitario} en las líneas de pedido para preservar el precio
en el momento de la compra, y la relación entre \texttt{pedidos} y
\texttt{direcciones} para gestionar el envío. El modelo entidad--relación
definitivo se presenta en la Figura~\ref{fig:modelo-er}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figs/prototipoBD.png}
    \caption{Prototipo de la base de datos relacional}
    \label{fig:prototipo-bd}
\end{figure}
\clearpage
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, keepaspectratio]{figs/ENTIDAD-RELACION.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth, keepaspectratio]{figs/LEYENDAENTIDAD-RELACION.png}
    \caption{Leyenda del modelo entidad--relación.}
    \label{fig:leyenda-er}
\end{figure}
\clearpage
