...
La Figura~\ref{fig:captura_checkout} muestra la pantalla de \textit{checkout} de la aplicación, donde el cliente revisa el resumen del pedido y selecciona el método de pago antes de confirmar la compra.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, keepaspectratio]{figs/captura_checkout.png}
    \caption{Pantalla de \textit{checkout} con resumen del pedido y selección del método de pago.}
    \label{fig:captura_checkout}
\end{figure}
...
Las Figuras~\ref{fig:captura_recuperar} y~\ref{fig:captura_email} ilustran el flujo completo: el formulario donde el usuario introduce su correo electrónico y el correo de recuperación recibido con el enlace de restablecimiento y el aviso de caducidad de una hora.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth, keepaspectratio]{figs/recuperacion1.png}
    \caption{Formulario de recuperación de contraseña en la aplicación web.}
    \label{fig:captura_recuperar}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth, keepaspectratio]{figs/recuperacion2.png}
    \caption{Correo electrónico de recuperación generado por el sistema, con el enlace de restablecimiento y el aviso de caducidad de una hora.}
    \label{fig:captura_email}
\end{figure}
...
La Figura~\ref{fig:captura_dashboard} muestra el panel de administración con las estadísticas en tiempo real: ingresos del mes actual frente al mes anterior, pedidos pendientes, stock crítico, historial de últimos pedidos y ranking de los cinco productos más vendidos.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, keepaspectratio]{figs/captura_admin_dashbord.png}
    \caption{Panel de administración con estadísticas de ventas, pedidos pendientes, stock crítico y productos más vendidos.}
...
La Figura~\ref{fig:captura_cloudinary} muestra el formulario de alta de producto en el panel de administración. El campo \textit{Imagen} permite seleccionar un archivo que se sube automáticamente a Cloudinary, almacenando la URL pública resultante en la base de datos.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, keepaspectratio]{figs/captura_admin_producto.png}
    \caption{Formulario de alta de producto en el panel de administración con el campo de subida de imagen a Cloudinary.}
    \label{fig:captura_cloudinary}
\end{figure}
...
Los tiempos de respuesta de los principales endpoints del backend se han medido con \textit{Postman}~\cite{postmanDocs} sobre el servidor local (\texttt{http://localhost:8080}). Cada petición se ha ejecutado con el sistema en estado de uso normal, con datos reales cargados en la base de datos.
