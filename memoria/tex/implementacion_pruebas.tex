...
Las pruebas de Lighthouse se han ejecutado en modo escritorio (\textit{desktop}) sobre las páginas principales de la aplicación. Se realizaron dos baterías de mediciones: la primera sobre el entorno de desarrollo (\texttt{ng serve}), y la segunda sobre el \textbf{build de producción} (\texttt{ng build}), que aplica minificación y \textit{tree-shaking} de JavaScript y CSS. Las métricas registradas son el índice de rendimiento general (\textit{Performance}), el tiempo hasta el primer renderizado de contenido (\textit{FCP}), el tiempo hasta el renderizado del elemento más grande (\textit{LCP}) y el tiempo de bloqueo total (\textit{TBT}).
...
\hline
\multicolumn{5}{|l|}{\textit{Build de desarrollo (ng serve)}} \\
\hline
Página de inicio   & 59 & 3,3 s & 5,5 s & 50 ms \\
\hline
Catálogo de productos & 57 & 3,3 s & 6,4 s & 100 ms \\
\hline
\multicolumn{5}{|l|}{\textit{Build de producción (ng build)}} \\
\hline
...
\hline
\end{tabular}
\caption{Resultados de Lighthouse en build de desarrollo y producción.}
\end{table}

Los resultados evidencian una mejora media de 30 puntos en el índice de rendimiento al pasar del entorno de desarrollo al build de producción, con reducciones del LCP de entre 3 y 5 segundos. Adicionalmente, durante las pruebas del catálogo se detectó que las imágenes de los productos penalizaban significativamente el LCP. Para corregirlo se aplicó \textit{lazy loading} mediante el atributo \texttt{loading="lazy"} y se añadieron dimensiones explícitas (\texttt{width} y \texttt{height}) en las tarjetas de producto, lo que redujo el LCP del catálogo de 2,9 s a 1,6 s y elevó su puntuación de 82 a 93.
